...
export default function Sidebar() {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`bg-[#1E2A38] h-screen transition-all duration-300 fixed left-0 top-0 z-40 ${
        ...
        <div className="p-6">
          <div className="flex items-center gap-3">
            ...
            {!isCollapsed && (
               <h3 className="text-xl font-bold text-white font-['Pacifico']">TaxiGo Admin </h3>
            )}
          </div>
        </div>
           ...
           .map((item) => {
            ...
            return (
              <Link 
                ...
                href={item.href}
                onClick={() => setIsMobileMenuOpen(false)}
                className={`flex items-center gap-3 px-4 py-3 rounded-lg mb-2 transition-all duration-200 hover:bg-white/10 group ${
        ...
        >
          <div className="w-5 h-5 flex items-center justify-center mx-auto">
            <i className={`ri-${isCollapsed ? 'arrow-right' : 'arrow-left'}-s-line text-lg`}></i>
          </div>
        </button>
      </div>
    </>
